...
class GitFileChangeAdapter(
	...
	private val _selectedFiles = mutableSetOf<String>()
	val selectedFiles: Set<String> get() = _selectedFiles
	...
	fun areAllSelected(): Boolean = selectablePaths.isNotEmpty() && _selectedFiles.containsAll(selectablePaths)

	/** Select every non-conflicted file. */
	fun selectAll() {
		_selectedFiles.addAll(selectablePaths)
		notifyItemRangeChanged(0, itemCount)
		onSelectionChanged(_selectedFiles.size)
	}
	...
	override fun onCurrentListChanged(
		...
		if (_selectedFiles.retainAll(currentPaths)) {
			notifyItemRangeChanged(0, itemCount)
			onSelectionChanged(_selectedFiles.size)
		}
	}
	...
	override fun onCreateViewHolder(
		...
		val binding =
			ItemGitFileChangeBinding.inflate(
	...
	inner class ViewHolder(
		private val binding: ItemGitFileChangeBinding,
		...
		fun bind(change: FileChange) {
			...
			if (isConflicted && _selectedFiles.remove(change.path)) {
				onSelectionChanged(_selectedFiles.size)
			}

			binding.checkbox.isChecked = _selectedFiles.contains(change.path)
			...
			binding.checkbox.setOnCheckedChangeListener { _, isChecked ->
				...
				if (isChecked) {
					_selectedFiles.add(changeAtPos.path)
				} else {
					_selectedFiles.remove(changeAtPos.path)
				}
				onSelectionChanged(_selectedFiles.size)
			}

...
class FileTreeFragment :
	...
	private val pluginRefreshRunnable =
		Runnable {
	...
	override fun onCreateView(
		...
		binding?.root?.doOnApplyWindowInsets { view, insets, _, _ ->
			insets
				...
				.apply { view.updatePadding(top = top + view.context.dpToPx(8f)) }
		}
		return binding!!.root
	}
	...
	private fun collectExpandedPaths(
		node: TreeNode,
	...
	private fun relistPreservingExpansion(
		node: TreeNode,
	...
	private fun createTreeView(node: TreeNode): AndroidTreeView? =
		if (context == null) {
			...
			AndroidTreeView(context, node, drawable.bg_ripple).also {
				fileTreeView = it
	...
	private fun tryRestoreState(
		rootNode: TreeNode,
		state: String? = viewModel.savedState,
	) {
	...
	private fun refreshNodeAfterDrop(
		targetNode: TreeNode?,
		targetFile: File,
	) {
		if (targetNode == null) {

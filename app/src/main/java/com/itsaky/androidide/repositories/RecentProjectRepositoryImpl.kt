...
class RecentProjectRepositoryImpl(
	private val recentProjectDao: RecentProjectDao,
) : RecentProjectRepository {
	override suspend fun insert(recentProject: RecentProject) = recentProjectDao.insert(recentProject)

	override suspend fun updateLanguage(
		location: String,
		language: String,
	) = recentProjectDao.updateLanguage(location, language)
}

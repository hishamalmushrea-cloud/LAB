...
internal class FabPositionCalculator {
	 ...
	 */
	fun getSafeDraggingBounds(
		parentView: ViewGroup,
		fabView: FloatingActionButton,
	): Rect {
		val defaultMargin = parentView.context.dpToPx(16f)
		val margins = resolvePhysicalMargins(parentView, fabView, defaultMargin)

		val insets =
			ViewCompat
				.getRootWindowInsets(parentView)
	...
	private fun resolvePhysicalMargins(
		...
		val layoutParams =
			fabView.layoutParams as? ViewGroup.MarginLayoutParams
		...
		val (resolvedLeft, resolvedRight) =
			if (isRtl) {
				end to start
			} else {
				start to end

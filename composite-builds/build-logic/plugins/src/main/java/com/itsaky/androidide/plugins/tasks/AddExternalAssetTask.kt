...
abstract class AddExternalAssetTask : DefaultTask() {
	...
	private fun AssetConfiguration.fetchSourceInto(cacheDir: File): File =
		when (this.source) {
			is AssetSource.External -> fetchExternalSourceInto(cacheDir)
		}

	private fun AssetConfiguration.fetchExternalSourceInto(cacheDir: File): File {
...
private fun AssetConfiguration.resolveDestFile(outputDir: File): File =
	when (this) {
...
private fun AssetConfiguration.resolveCacheFile(cacheDir: File): File =
	when (this) {

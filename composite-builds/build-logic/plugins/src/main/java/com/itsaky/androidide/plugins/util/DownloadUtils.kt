...
object DownloadUtils {
    ...
    internal fun downloadFile(
        ...
        val parent = absoluteDestination.parentFile
    ...
    private fun validateContentRange(
        ...
        val match = contentRangePattern.matchEntire(value)
    ...
    private fun rejectUnsafeFile(file: File, description: String) {
    ...
    private fun isExpectedFile(file: File, expectedSize: Long, checksum: String): Boolean =
        file.isFile &&
            file.length() == expectedSize &&
            MessageDigest.getInstance("SHA-256").sha256(file) == checksum

    private fun moveAtomically(partial: File, destination: File) {
        try {
            Files.move(
                partial.toPath(),
                destination.toPath(),

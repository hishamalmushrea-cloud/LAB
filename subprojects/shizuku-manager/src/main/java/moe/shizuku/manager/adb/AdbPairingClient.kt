...
private const val K_CURRENT_KEY_HEADER_VERSION = 1.toByte()
private const val K_MIN_SUPPORTED_KEY_HEADER_VERSION = 1.toByte()
private const val K_MAX_SUPPORTED_KEY_HEADER_VERSION = 1.toByte()
private const val K_MAX_PEER_INFO_SIZE = 8192
private const val K_MAX_PAYLOAD_SIZE = K_MAX_PEER_INFO_SIZE * 2

private const val K_EXPORTED_KEY_LABEL = "adb-label\u0000"
private const val K_EXPORTED_KEY_SIZE = 64

private const val K_PAIRING_PACKET_HEADER_SIZE = 6

private class PeerInfo(
	val type: Byte,
	data: ByteArray,
) {
	val data = ByteArray(K_MAX_PEER_INFO_SIZE - 1)

	init {
		data.copyInto(this.data, 0, 0, data.size.coerceAtMost(K_MAX_PEER_INFO_SIZE - 1))
	}
...
private class PairingPacketHeader(
	...
	companion object {
		fun readFrom(buffer: ByteBuffer): PairingPacketHeader? {
			...
			if (version < K_MIN_SUPPORTED_KEY_HEADER_VERSION || version > K_MAX_SUPPORTED_KEY_HEADER_VERSION) {
				Log.e(TAG, "PairingPacketHeader version mismatch (us=$K_CURRENT_KEY_HEADER_VERSION them=$version)")
				return null
			}
			...
			if (payload <= 0 || payload > K_MAX_PAYLOAD_SIZE) {
...
class AdbPairingClient(
	...
	private fun setupTlsConnection() {
		...
		val keyMaterial = Conscrypt.exportKeyingMaterial(sslSocket, K_EXPORTED_KEY_LABEL, null, K_EXPORTED_KEY_SIZE)
	...
	private fun createHeader(
		...
		payloadSize: Int,
	): PairingPacketHeader = PairingPacketHeader(K_CURRENT_KEY_HEADER_VERSION, type.value, payloadSize)

	private fun readHeader(): PairingPacketHeader? {
		val bytes = ByteArray(K_PAIRING_PACKET_HEADER_SIZE)
	...
	private fun writeHeader(
		...
		val buffer = ByteBuffer.allocate(K_PAIRING_PACKET_HEADER_SIZE).order(ByteOrder.BIG_ENDIAN)
	...
	private fun doExchangePeerInfo(): Boolean {
		val buf = ByteBuffer.allocate(K_MAX_PEER_INFO_SIZE).order(ByteOrder.BIG_ENDIAN)
		...
		if (decrypted.size != K_MAX_PEER_INFO_SIZE) {
			Log.e(TAG, "Got size=${decrypted.size} PeerInfo.size=$K_MAX_PEER_INFO_SIZE")
			return false
		}

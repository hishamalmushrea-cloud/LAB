...
class UtilTest {
	...
	@Test
	fun `test projectLanguageParameter excludes Language Unknown by default and with custom filter`() {
		val defaultParam = projectLanguageParameter()
		assertThat(defaultParam.filter?.invoke(Language.Java)).isTrue()
		assertThat(defaultParam.filter?.invoke(Language.Kotlin)).isTrue()
		assertThat(defaultParam.filter?.invoke(Language.Unknown)).isFalse()

		val customParam =
			projectLanguageParameter {
				filter = { it == Language.Java }
			}
		assertThat(customParam.filter?.invoke(Language.Java)).isTrue()
		assertThat(customParam.filter?.invoke(Language.Kotlin)).isFalse()
		assertThat(customParam.filter?.invoke(Language.Unknown)).isFalse()

		val permissiveParam =
			projectLanguageParameter {
				filter = { true }
			}
		assertThat(permissiveParam.filter?.invoke(Language.Java)).isTrue()
		assertThat(permissiveParam.filter?.invoke(Language.Kotlin)).isTrue()
		assertThat(permissiveParam.filter?.invoke(Language.Unknown)).isFalse()
	}
}

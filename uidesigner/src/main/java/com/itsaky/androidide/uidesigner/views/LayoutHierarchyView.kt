...
class LayoutHierarchyView
	...
	constructor(
		...
		private val pointRadius = context.dpToPx(3f).toFloat()
		private val dp8 = context.dpToPx(8f)
		...
		init {
			...
			paint.strokeWidth = context.dpToPx(1f).toFloat()
			paint.isAntiAlias = true
		}

		fun setupWithView(
			view: IView,
			onClick: ((IView) -> Unit)? = null,
		) {
			removeAllViews()
			textToIView.clear()
		...
		class HierarchyText(
			...
			init {
				...
				layoutParams =
					MarginLayoutParams(LayoutParams.MATCH_PARENT, context.dpToPx(48f)).apply {
